...
func (a *Accounts) DeleteAccount(password string) error {
	...
	db := cockroach.GetDB()
	if err := db.Debug().Table(AccountsTable).
		Where("username = ?", a.Username).First(&v).Error; err != nil {
		return err
	}
...
// EditPassword ...
func (a *Accounts) EditPassword(newPassword string) error {
	v := NewAuthorization(a.Username, a.Password)
	...
	if err := bcrypt.CompareHashAndPassword([]byte(v.Password), []byte(a.Password)); err != nil {
		return errors.Errorf("PASSWORD_VERIFICATION_FAILED")
	}
