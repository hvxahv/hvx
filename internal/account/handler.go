...
func (s *server) Create(ctx context.Context, in *pb.CreateAccountData) (*pb.AccountsReply, error) {
	...
	a, err := NewActors(in.Username, "", "Person").Create()
	if err != nil {
		log.Println(err)
	}

	if err := NewAccounts(in.Username, in.Mail, in.Password, a.ID).Create(); err != nil {
		log.Panicln(err)
	}
...
func (s *server) SignIn(ctx context.Context, in *pb.AuthData) (*pb.SignInReply, error) {
	a := NewAuthenticate(in.Username, in.Password)
...
func (s *server) FindActorByAccountsUsername(ctx context.Context, in *pb.AccountUsername) (*pb.ActorData, error) {
	...
	a, err := a.GetActorByAccountUsername()
	if err != nil {
		return nil, err
	}
	return &pb.ActorData{
