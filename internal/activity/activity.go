...
func (i *Activities) Handler() {
	...
	case "Follow":
		if err := NewInboxes(i.AccountID, i.FromID, i.ActivityType, i.ActivityID, string(i.Body)).Create(); err != nil {
			log.Println(err)
			return
		}
	case "Undo":
		undo := activitypub.Undo{}
		if err := json.Unmarshal(i.Body, &undo); err != nil {
			fmt.Println(err)
			return
		}

		if err := NewInboxesActorID(undo.Object.Id).DeleteByActivityID(); err != nil {
			log.Println(err)
			return
		}
