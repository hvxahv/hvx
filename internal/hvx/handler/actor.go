...
func GetActorHandler(c *gin.Context) {
	...
	acct, err := account.NewActorsPreferredUsername(name).GetActorByAccountUsername()
	if err != nil {
		return
	}
...
func NewChannelActor(a *account.Actors) *activitypub.Actor {
	var (
...
func NewActor(a *account.Actors) *activitypub.Actor {
	var (
		addr = viper.GetString("localhost")
