...
func SignUpHandler(c *gin.Context) {
	...
	//})
	//if err != nil {
	//	log.Printf("failed to send message to account hvx: %v", err)
	//}
	...
	a, err := account.NewActors(username, publicKey, "Person").Create()
	if err != nil {
		log.Println(err)
		return
	}

	if err := account.NewAccounts(username, mail, password, a.ID).Create(); err != nil {
		log.Panicln(err)
		return
	}
...
func UploadAvatar(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(500, gin.H{
			"status":  500,
			"message": "hvx receiving file error!",
		})
		return
	}
