...
func Req2(da string, pk []byte) {
	...
	p := gin.H{
		...
		"id": "https://activitypub.disism.com/create-" + idr,
		"type": "Like",
		"actor": "https://activitypub.disism.com/actor",
	...
	httpsig.SignRequest("https://activitypub.disism.com/actor", block, req, byterData)
	ctx, cancel := context.WithTimeout(context.Background(), 1*time.Minute)
	defer cancel()
	req = req.WithContext(ctx)
	res, err := client.Do(req)
